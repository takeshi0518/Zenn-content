...
僕が同じ機能を持つブログアプリを作り、比較検証して感じた違いは沢山あります。
...
// 全部JavaScript/JSX
export default async function PostsPage() {
  const posts = await supabase.from('posts').select('*');  // JavaScript
  return {posts?.map(...)};  // JSX
}
```
...
MVC と Next.js のもう一つの大きな違いは、「どこで何を実行させるか」という計算リソースの配分です。

#### MVC 時代

MVC では、サーバーで HTML を生成していました。
もしブラウザ側の動き（DOM 操作）が必要なら、
重い JavaScript ライブラリーをページ全体に読み込ませるしかありませんでした。

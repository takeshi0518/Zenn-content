---
title: 'Next.jsは「近道」か「迷路」か。迷子の初学者がWebの歴史を遡り、Next.jsの本質が見えてきた話'
emoji: '🔄'
type: 'tech' # tech: 技術記事 / idea: アイデア
topics: ['nextjs', 'react', 'mvc', 'フロントエンド', '初心者']
published: true
---
...
jQuery による**命令的**な DOM 操作は、HTML の構造が変わるたびに JavaScript 側も修正が必要になり、バグの温床になります。
...
![Next.jsブログアプリのスクリーンショット](/images/blog-nextjs-screen-shot.png)
...
ここで重要なのは**MVC も同じことをしていた**。

もちろんなことですが、MVC モデルも**サーバーで HTML を生成**していました。
...
ハイドレーション(HTML に JS を紐付ける作業)に時間がかかる。
...
Next.js の**Server Actions**という機能を使うと、今回実装した記事作成フォームは**JavaScript なしでも動作します**。
...
![MVCとNext.jsの比較図](/images/mvc-vs-nextjs.png)
...
:::message
**共通処理の扱いについて**
凝集にこだわり、ロジックをページに閉じ込めすぎると、他のページでも同じデータ
を使いたい時にコードが重複する問題があります。
例えば、記事データを「一覧ページ」「詳細ページ」「トップページ」で使いたい場合、それぞれにデータ取得処理を書くと保守性が下がります。

複数のページで同じデータを扱う場合、
データ取得処理を`actions/`ディレクトリや
カスタムフック、状態管理ライブラリなどに切り出すことで、
DRY 原則を守りつつ Colocation を実現できます。

```typescript
// hooks/usePosts.ts（共通のロジック）
...
これなら、各ページに凝集させる良さを活かしつつ、
中身の処理を一箇所にまとめて再利用できます。
:::
...
MVC 時代は言語による物理的制約がありました。サーバーサイドのロジック(JavaScript/Ruby/PHP 等)と、表示テンプレート(EJS/ERB/Blade 等)は異なる言語であり、言語が違う以上、それらを**物理的に分離し、データを「注入」する形をとらざるを得ませんでした**。
...
しかし Next.js はその常識を塗り替え、１つのコンポーネントの中でサーバー(データ取得)とクライアント(操作)を自在に融合させました。
これはまさに、Web 開発のパラダイムシフトだと言えます。

### 2.「関心の分離」の再定義

MVC モデルは３つの役割を「技術(ファイル)」によって物理的に分離します。対して Next.js は、UI とロジックを「機能(コンポーネント)」として１箇所に集約します。実際に両方で実装してみた結果、現代の複雑な UI 開発において、**「意味のある単位でのカプセル化」**がいかに強力であるかを肌で感じることができました。
...
※学習中である僕が偉そうなこと言ってすみません。

- MVC から学び、不自由さを知ってから Next.js の恩恵を受けるのか

- Next.js から学び、作りながら「なぜ？」を遡って本質を追いかけるのか

どちらが正解かは分かりません。しかしそれが、**「近道」になるのか「迷路」になるのかは、学習者が本質を理解しようとする姿勢次第**なのだと僕は思います。

...
- このコンポーネントは JavaScript はブラウザに送られない
...
export function DeleteButton({ postId }: { postId: number }) {
  //このコンポーネントのJavaScriptだけがブラウザに送られる
}
```
...
- 初回ロードが速い
- JavaScript のサイズが小さい
- ページごとに必要なコードだけダウンロード

#### 3. JavaScript への依存 → ハイブリッドアプローチ

**SPA の問題**

JavaScript が無効だと、なにも表示されませんでした。

**Next.js の解決策**

Server Component で生成された HTML は、**JavaScript なしでも表示されます**。

```
① 初回アクセス
   → Server Componentが完成したHTMLを返す
   → JavaScriptなしでも表示できる

② その後のインタラクション
   → 必要な部分だけClient Component
   → JavaScriptが必要 (削除ボタンなど)
```

**つまり**

- 基本的なコンテンツ表示: JavaScript なしでも OK
- インタラクティブな機能: JavaScript が必要

**プログレッシブエンハンスメント**

基本機能(HTML)を提供し、JavaScript で拡張する、という Web の基本原則に立ち返りました。

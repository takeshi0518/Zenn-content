...
実は、MVC アーキテクチャも**サーバーで HTML を生成**していました。
...
## 6. MVC アーキテクチャと Next.js の決定的な違い

### 6-1. 関心の分離の再定義

MVC と Next.js どちらも「関心の分離」を重視しています。
しかし**意味が全く違います**。

#### MVC の関心の分離 → 技術で分離

MVC は、アプリケーションを技術的な役割で分離しています。
...
//Controller (controllers/postsController.js) - 制御
exports.index = async (req, res) => {
  const posts = await Post.findAll();
  res.render('posts/index', { posts });
};

//View (views/posts/index.ejs) - 表示
<% posts.forEach(post => { %>
  <%= post.title %>
<% }) %>
```
...
「記事一覧」という１つの機能が、**3 つのファイルに分散**しています。

#### Next.js の関心の分離 → 機能で統合
...
// app/posts/page.tsx
export default async function PostsPage() {
  const posts = await supabase.from('posts').select('id, title, content');  // データ取得
  return {posts?.map(...)};  // 表示
}
```

「記事一覧」という１つの機能が、**1 つのファイルに統合**されています。
...
| **分離の軸**   | 技術的役割                 | 機能的まとまり             |
| **ファイル数** | 3 ファイル                 | 1 ファイル                 |
| **メリット**   | 役割が明確、分業しやすい   | 変更しやすい、見通しが良い |
| **デメリット** | ファイル跨ぎ、変更コスト高 | 複雑化すると肥大化         |

**同じ「関心の分離」でも、時代とともに意味が変わった。**

---

#### この記事を書いて気づいたこと

「関心の分離」という言葉は 1 つの意味ではありませんでした。

- **MVC 時代**: 技術的な責務で分離することが「正しい」
- **Next.js 時代**: 機能的なまとまりで整理することが「正しい」

**技術の進化 = 常識の変化**

これが、実装を通じて最も学んだことの 1 つです。

### 6-2. ファイルベースルーティング

ルーティングの定義方法も、MVC と Next.js では根本的に違います。

#### MVC のルーティング

MVC では、ルーティングを**明示的に定義**します。
...
Next.js では、**ファイル構造 = URL 構造**になります。
...
「URLは/posts/123 だから...」
  ↓
「app/posts/[id]/page.tsx にあるはず」
```

URL → ファイルパスが直結

MVC では、routes → Controller → View と 3 ファイルをまたぐ必要がありました。
...
**どちらが優れている、ではなく適材適所だということが分かります。**

### 6-3. Next.js の設計思想 MVC への回帰
